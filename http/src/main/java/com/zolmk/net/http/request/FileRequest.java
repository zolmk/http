...
public interface FileRequest {
    ...
    @Streaming
    @GET("{dir}/{filename}")
    Observable<ResponseBody> downloadFileByFileName(@Path("dir") String dir,@Path("filename") String fileName);
    ...
    @Multipart
    @POST("{url}")
    Observable<BasicBean> uploadFiles(@Path("url")String path,@PartMap Map<String, RequestBody> params);
